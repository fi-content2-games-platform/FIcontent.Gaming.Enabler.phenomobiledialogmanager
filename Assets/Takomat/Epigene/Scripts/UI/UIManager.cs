...
namespace Epigene.UI
{
	...
	public sealed class UIManager
	{
			...
			foreach (RaycastHit2D hit in hits)
			{
				if(hit.collider)
				{
					...
					else 
					{
						...
						SortingOrder sortingOrder = temp.GetComponent<SortingOrder>();
		...
		/// <summmary>
		/// Get instance of a button by name
		/// </summary>
		public UIObject Get(string name)
		{
			...
			if(obj != null)
			{
				uiObj = UIManager.Instance.Get(obj.GetInstanceID());
			}
	    ...
	    public void LoadResources(string fileName, GameObject gameObject = null)
		{
			...
			foreach(Dictionary<string,object> d in ((List<object>)dict["screens"]))
			{
				...
				screenObject.name = id;
				screenObject.SetActive(false);

				string scriptName = d["script"].ToString();
#if UNITY5
				UIScreen screen = UnityEngineInternal.APIUpdaterRuntimeServices.AddComponent(screenObject, "Assets/Takomat/epigenecore/Scripts/UI/UIManager.cs (1497,23)", scriptName) as UIScreen;
#else
		...
		public GameObject GetHud(string id, string prefab = null)
		{
			...
			hud.name = id;
			hud.transform.parent = hudParent.transform;
			huds.Add(id,hud);
