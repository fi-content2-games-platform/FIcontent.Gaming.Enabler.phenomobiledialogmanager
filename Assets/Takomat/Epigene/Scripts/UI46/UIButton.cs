...
//------------------------------------------------------------------------------
			...
			:base(dict)
		{
			...
			//Parse(dict);
			// Debug.Log("<color=orange> *** image: " + this.image + ", sprites.Length:" +this.sprites +"</color>");
		}//ctor()


		void onClickListener(string eventId, string gameObjectName)
		{
			// Debug.Log("<color=red>**** " + eventId + " ****</color>");
			if(eventId.Length > 0)
				GameManager.Instance.Event("SCREEN", eventId, gameObjectName);
		}
		...
		protected override void Parse(Dictionary<string,object> dict)
		{
			...
			if (dict["type"].ToString() == "togglebutton")
				return;
			...
			if(dict.ContainsKey("event"))
			{
				...
				b.onClick.AddListener(() => onClickListener(this.eventId, this.gameObject.name));
			...
			// Debug.Log("<color=orange>image: " + this.image + ", sprites.Length:" +sprites.Length +"</color>");
			b.targetGraphic = GameObject.GetComponent<UnityEngine.UI.Image>();

			UnityEngine.UI.SpriteState state = b.spriteState;
			if (sprites != null && sprites.Length >= 2)
				state.pressedSprite = sprites[(int)UIButtonState.PUSH];
			if (sprites != null && sprites.Length >= 3)
				state.highlightedSprite = sprites[(int)UIButtonState.OVER];
			if (sprites != null && sprites.Length >= 4)
				state.disabledSprite = sprites[(int)UIButtonState.DISABLED];

			b.spriteState = state;

			UnityEngine.UI.Navigation nav = b.navigation;
